...
<html>
<head>
	<title>ubah menu da-resto</title>
...
<div class="container">
	<h1>ubah menu</h1><br>
		@foreach($data as $dt)
		<form class="row g-3" action="{{ url('menu_update', $dt->id) }}" method="post" enctype="multipart/form-data">
			{{ csrf_field() }}	
            {{ method_field('PATCH') }}
    				<label for="nama">Nama Menu</label>
    				<input type="text" id="nama" name="nama_menu" placeholder="nama menu" value="{{$dt->nama_menu}}" required>
    				...
    				<textarea id="desc" name="description" placeholder="Description" required>{{$dt->description}}</textarea>

					<label for="harga">price</label>
					<input type="number" id="harga" name="price" placeholder="price" value="{{$dt->price}}" required>
					<div class="navi">
